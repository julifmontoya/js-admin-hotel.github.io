...
async function getHotelList(url) {
  const resp = await fetch(url + /hotels-list/, {
    method: "GET",
  });
  const hotelList = await resp.json();
  return hotelList;
}

function showHotels(hotelList) {
  const table = document.getElementById("table_hotel_list");
  let tbody = "<tbody>";
  for (let i = 0; i < hotelList.length; i++) {
    tbody += `
    <tr>
    <td> ${hotelList[i].id} </td>
    <td> ${hotelList[i].name} </td>
    <td>
    <a onclick='sendParams(${hotelList[i].id})' title="Editar">
     <svg
     version="1.1"
   ...
   >
     ...
     <a onclick='deleteHotel(${hotelList[i].id})' title="Eliminar">
     <svg
       height="512pt"
...
async function sendParams(id) {
  window.location.href = `hotel.html?hotel=${id}`;
}

async function deleteHotel(id) {
  if (confirm("Esta seguro de eliminar? No podra revertir esto!")) {
    await fetch(url + /hotels-delete/ + id + "/", {
      method: "DELETE",
    });
  }
  else {
...
async function main() {
  const hotelList = await getHotelList(url);
